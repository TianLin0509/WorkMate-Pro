...
2026-09-18：四步向导主体、下一步转移及错误恢复统一按测试 PID 枚举窗口，等待目标步骤 AutomationId 与可用操作按钮（保留 20 秒截止），不再以 `MainWindowHandle` 非零或固定延时判断就绪。超时诊断保留窗口候选；树快照显式记录元素数及 empty 状态，避免空列表写日志异常掩盖原失败。
